...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR ROZER — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { rozer_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.rozer_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t a ROZER plan file'))
  }
