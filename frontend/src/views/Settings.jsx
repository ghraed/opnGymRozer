...
import { api, passwordRegister, IS_ANDROID } from '../lib/api.js'
...
export default function Settings() {
  ...
  const doExport = async () => {
    ...
    const name = 'rozer-backup-' + todayISO() + '.json'
  ...
  const doImport = ev => {
    ...
    rd.onload = () => {
      try {
        ...
        if (!data.workouts || !data.routines) throw new Error('not a ROZER backup')
  ...
  return <div className="narrow">
    ...
    <Section title={MOBILE ? t('Your data') : DEMO ? t('Demo') : t('Account')}>
      {MOBILE ? <>
        ...
        <Row icon="sparkles" iconTint="var(--acc)" title={t('Create account')} subtitle={t('Keeps your data safe and separate per person.')} accessory="chevron" onClick={registerHere} />
        <Row icon="person" iconTint="var(--blue)" title={t('Sign in')} accessory="chevron" onClick={() => nav('/')} />
      </>}
    </Section>
    ...
    <Section title={t('Data')}>
      <Row icon="sparkles" iconTint="var(--acc)" title={t('Choose workout program')} accessory="chevron" onClick={loadStarterPlan} />
    ...
    {!MOBILE && <Section title={t('Tip')}>
      <Row icon="lightbulb" iconTint="var(--yellow)"
        ...
        subtitle={t('to install ROZER as a full-screen app.') + ' ' + (user ? t('Your data syncs with your profile — sign in anywhere to see it.') : t('Guest data stays on this device — export a backup now and then!'))} />
    </Section>}
  ...
  return <>
    <Section
      ...
      footer={on && S.reminder?.on
        ...
        : null}
    >
      <Row icon="bell" iconTint="var(--red)" title={t('Push notifications')} subtitle={t('Rest-timer alerts, even if ROZER is closed.')}>
...
function RegisterInline({ close, setUser, pushState, pullState, toast }) {
  ...
  return <>
    ...
    <div style={{ height: 12 }} /><Button variant="primary" onClick={go}>{t('Create account')}</Button>
  </>
}
